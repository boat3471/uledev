...
var configData = require(ule.uledevEventJsonPath);
...
var createEventHandler = {
	...
	create: function(type, name, date){
		...
		var map = configData;
		var ownConfigExists = fs.existsSync(ule.uledevEventJsonOwnPath);
		if(ownConfigExists) map = require(ule.uledevEventJsonOwnPath);
		var config = map[type];
...
function installEvent(){
	var exists = fs.existsSync(ule.uledevEventJsonOwnPath);
	if(exists) return outputConfig('\r\n  活动配置文件已存在！');
	var targetContent = JSON.stringify(configData, null, 4);
	fs.writeFileSync(ule.uledevEventJsonOwnPath, targetContent);
	console.info('\r\n  活动配置文件创建成功: ', ule.uledevEventJsonOwnPath);
	outputConfig();
}
